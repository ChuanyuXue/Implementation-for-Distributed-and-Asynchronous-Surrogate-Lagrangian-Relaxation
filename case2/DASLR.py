...
class Coordinator(Process):
    ...
    '''
    def __init__(self, M = 70, r = 0.05, L_star = 97887, time = 100, threshold = 0.1):
        super().__init__()
        self._M = M
        self._r = r
        self._L_star = L_star
    ...
    def init(self, para, subproblems):
        ...
        self.solutions = Manager().list()
        self.lambdas = Manager().list()
        ...
        self._s = (1 / 21) *  (self._L_star - self._surrogate_dual) / (np.linalg.norm(self._g))**2
#         self._s = 0.01
...
class Solver(Process):
    ...
    def solve(self,):
        model = cpx.Model(name="MIP Model " + self._name)
